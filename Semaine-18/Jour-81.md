...
- [ ] **C# (avec ASP.NET Core Test Host) :**
  ...
  - [x] Utiliser `WebApplicationFactory` pour démarrer l'application en mémoire.
  - [x] Envoyer des requêtes HTTP au serveur de test avec `HttpClient`.
  - [x] Interagir avec une base de données de test (ex: EF Core In-Memory Database ou LocalDB).
